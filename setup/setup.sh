...
if command -v /usr/local/bin/python > /dev/null 2>&1 && [[ "Python ${component[python]}" == "$(/usr/local/bin/python --version 2>&1)" ]]; then prereqs[python]="Expected"; else prereqs[python]="Unknown"; fi
...
case "${OSTYPE}" in
    ...
    linux*)
        if [[ -r /etc/centos-release ]]; then
            ...
            # If a new version of CentOS 6 comes out and you want to support it, revisit the check below in the "Setup Metron" block
            if [[ "${OS[distro]}" == "CentOS" && ("${OS[version]}" == "6.8" || "${OS[version]}" =~ "7.3") ]]; then
                if [[ "${component[metron]}" == "master" && "${OS[version]}" == "6.8" ]]; then
...
if [[ "${usetheforce}" != "1" ]]; then
    ...
    while [ -z "${prompt}" ]; do
        read -p "This script is intended to be run on a fresh CentOS 6.8 or 7.3 installation and may have unintended side effects otherwise.  Do you want to continue (y/N)? " prompt
        case "${prompt}" in
            [yY]|[yY][eE][sS])
...
# Setup docker
if [[ "${OS[distro]}" == "CentOS" && "${OS[version]}" == "7.3" ]]; then
    # Currently there isn't a specific version of docker that is required, so installing the latest ce
